...
type ResBody struct {
	HttpCode int    `json:"-"`
	Code     string `json:"code" example:"200/500/S1000/E1000"` // 响应码
...
func Validate(err error) ResBody {
	return jsonResult(http.StatusBadRequest, constant.ValidateFail, err.Error(), nil, false)
}

// Error 500 系统异常
func Error() ResBody {
	return jsonResult(http.StatusInternalServerError, constant.SysFail, "", nil, false)
}
