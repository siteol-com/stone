...
// 获取用户的联动权限
func getUserDataPermission(deptId uint64, permissionType uint8, traceID string) (uint8, []uint64) {
	var dept *platDb.Dept
	// 如果数据权限跟随部门，读取部门数据权限
	if permissionType == constant.PermissionTypeDeptFellow {
		dept, err := (&platDb.Dept{Id: deptId}).FindOne()
...
func recursionDept(dept *platDb.Dept, deptIds []uint64) []uint64 {
	...
	// 查询子部门
	childDeptList, err := (&platDb.Dept{Pid: dept.Id}).FindList()
	// 查询出错或者不再有子部门，结束递归
